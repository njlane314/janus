...
Recombination::Recombination(int ionisations, int excitations, double visible_linear_transfer, double electric_field)
{
...
double Recombination::compute_recombination_factor(double visible_linear_transfer, double electric_field)
{
    ...
    auto EscapeRecomb = [&]() -> G4double
    {
        ...
        {
            if (visible_linear_transfer < 1) //, then the fit becomes unphysical and
            {
                return 0;
            }
            else
    ...
    double R = BirksRecomb() + EscapeRecomb();
